...
export class OperationClaimDeleteComponent implements OnInit {
  ...
  delete() {
    if (this.operationClaimDeleteForm.valid) {
      let operationClaimModel = Object.assign({}, this.operationClaimDeleteForm.value);
      this.operationClaimService.deleteOperationClaim(operationClaimModel).subscribe(
        (response) => {

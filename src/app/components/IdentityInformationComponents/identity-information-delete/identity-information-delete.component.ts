...
export class IdentityInformationDeleteComponent implements OnInit {
  ...
  delete() {
    if (this.identityInformationDeleteForm.valid) {
      let identityInformationModel = Object.assign({}, this.identityInformationDeleteForm.value);
      this.identityInformationService.deleteIdentityInformation(identityInformationModel).subscribe(
        (response) => {
